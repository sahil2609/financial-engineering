...
def asian_option_pricing(iter = 1000,r=0.05,sigma=0.2,S0=100,K=105,n = 252):
    ...
    for i in range(iter):
        ...
        control_variate_put.append(math.exp(-r*n*dt) * max(K - S[len(S) - 1], 0))
        control_variate_call.append(math.exp(-r*n*dt) * max(S[len(S) - 1] - K, 0))
    asian_call = variance_reduction(asian_call,control_variate_call,r,n,dt)
    asian_put = variance_reduction(asian_put,control_variate_put,r,n,dt)
...
for k in K:
    print(f'For K = {k}')
    call_mean,call_var,put_mean,put_var = asian_option_pricing(1000,0.05,0.2,100,k,252)
